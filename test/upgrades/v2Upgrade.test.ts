...
describe("Upgrade V1 to V2", () => {
  it("successfully upgrade the proxy contract", async () => {
    ...
    const upgraded = await upgrades.upgradeProxy(instance.address, SpritzPayV2);

    /**
     * Validate major storage slots
     */
    paymentRecipient = await upgraded.paymentRecipient();
    expect(paymentRecipient).to.eq(RECIPIENT);

    swapTarget = await upgraded.swapTarget();
    expect(swapTarget).to.eq(SWAP_TARGET);
    ...
    await upgraded.connect(admin).setV3SwapTarget(V3_SWAP_TARGET);
